...
router.delete("/", upload.none(), function(request, response) {
    ...
    let removingAccount = accounts.find({id: request.body.account_id}).value();// нахождение нужного удаляемого счёта
    if(removingAccount){// если удаляемый аккаунт существует
        accounts.remove({id: request.body.account_id}).write();// удалить и перезаписать аккаунт
        transactions.remove({account_id: request.body.account_id}).write(); // удалить связанные транзакции и перезаписать
